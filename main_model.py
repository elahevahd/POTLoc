...
class Model(nn.Module):
    ...
    def forward(self, final_dict,is_training, inference_dict=None):

        if not inference_dict:
            self.inference_dict = self.config.inference
        else:
    ...
    def inference(self, video_list, fpn_masks, out_cls_logits, out_cls_logits_fuse):
        ...
        vid_num_seg = [x['vid_num_seg'] for x in video_list][0]
        ...
        _lambda = self.inference_dict["_lambda"]
        gamma = self.inference_dict["gamma"]
        ######################################################
        fpn_pred_list = []
        proposal_dict = {}
        for fpn_idx in range(len(self.fpn_strides)): 
            ...
            for thresh_idx in range(len(act_thresh_cas)):
                ...
                for c in range(len(pred)):
                    ...
                    seg_list.append(pos)                                
                proposals = main_utils.get_proposal_oic(seg_list, cas_temp, score_np, pred, temp_scale, self.fpn_strides[fpn_idx], self.feature_fps, vid_num_seg, self.feat_stride, self.num_frames,_lambda, gamma)                                
