...
def get_proposal_oic(seg_list, cas_temp, score_np, c_pred, temp_scale, fpn_scale,  feature_fps, vid_num_seg, feat_stride, num_frames, _lambda, gamma):
    temp = []
    for i in range(len(seg_list)):
        ...
        if temp_list.any():
            ...
            for j in range(len(grouped_temp_list)):
                ...
                t_factor = float(feat_stride*fpn_scale)/(temp_scale*feature_fps)
                t_start = start_index*t_factor
                t_end = end_index*t_factor

                # # delta = (num_frames/2)
                # delta = 0
                # start_index = float(start_index / temp_scale)*fpn_scale* feat_stride
                # end_index = float(end_index / temp_scale)*fpn_scale* feat_stride
                # t_start = float(start_index + delta)/ feature_fps  
                # t_end = float(end_index + delta)/feature_fps 
