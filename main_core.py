...
def truncate_feats(feats, video_item , feat_stride , feat_offset, max_seq_len, crop_ratio=None, max_num_trials=500):
    ...
    for _ in range(max_num_trials):
        ...
        for instance_idx in range(num_points):
            ...
            curr_point_indx = (curr_point*fps) /feat_stride- feat_offset  #feature grid 
            #------------------------------------------------------------------------------
            curr_start, curr_end = video_item["proposals"][instance_idx] #in seconds 
            curr_start_indx = (curr_start*fps) /feat_stride- feat_offset  #feature grid 
            curr_end_indx = (curr_end*fps) /feat_stride- feat_offset  #feature grid 
            #------------------------------------------------------------------------------
...
class ThumosFeature(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        filename = self.feat_dir+"/{}.npy".format(vid_name)        
        feats = np.load(filename).astype(np.float32) #T x C 
        feats = torch.from_numpy(np.ascontiguousarray(feats.transpose())) # C x T
        vid_num_seg = feats.shape[1] 
        #########################################################################
        if self.is_training:
            feats, final_points, final_labels, final_proposals = truncate_feats(feats, video_item , self.feat_stride , self.feat_offset, self.max_seq_len, self.crop_ratio)
...
def train(args):
    ...
    train_dataset = ThumosFeature(is_training = True)
...
def test(args):
    ...
    del checkpoint
    #######################################################################
    val_dataset = ThumosFeature(is_training = False)
    ...
    anet_detection = ANETdetection(val_dataset.json_file, proposals_path,subset="Test", tiou_thresholds=tIoU_thresh,verbose=False, check_status=False)                                                                
