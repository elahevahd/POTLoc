...
def check_performance(props_path,gt_path, mode):   
    tIoU_thresh = np.linspace(0.1, 0.7, 7)        
    anet_detection = ANETdetection(gt_path, props_path,
                                subset=mode, tiou_thresholds=tIoU_thresh,
...
def get_proposal_oic(tList, wtcam, final_score, c_pred, scale, feature_fps, feat_stride, window_len, _lambda, gamma):
    temp = []
    for i in range(len(tList)):
        ...
        if temp_list.any():
            ...
            for j in range(len(grouped_temp_list)):
                ...
                t_factor = float(feat_stride)/(scale*feature_fps)

                t_start = start_index*t_factor
                t_end = end_index*t_factor
...
class ThumosFeature(data.Dataset):
    def __init__(self, data_path, feat_stride, feat_offset, feature_fps, mode, seed=-1):
        if seed >= 0:
            set_seed(seed)
    ...
    def __getitem__(self, index):
        ...
        ################################### READ FEATURES ###############################        
        # feature= np.load(os.path.join(self.feature_path,vid_name + '.npy')).astype(np.float32) #stride 4

        rgb_path = os.path.join(self.feature_path, self.mode, 'rgb', vid_name + '.npy')
        rgb_feature = np.load(rgb_path).astype(np.float32)
        flow_path = os.path.join(self.feature_path, self.mode, 'flow', vid_name + '.npy')
        ...
        for key in temp_df['point'].keys():            
            ...
            t_factor = self.feature_fps/float(self.fps_dict[vid_name] * self.feat_stride)
            point_indx = int(point* t_factor) 
            point_indx = min(point_indx, vid_num_seg-1)
            ##################################################################
            temp_anno[point_indx][class_idx] = 1
        temp_anno = torch.from_numpy(temp_anno)

        return index, feature, label, temp_anno, self.vid_list[index], vid_num_seg
...
def train(config, net, loader_iter, optimizer, criterion, logger, step):
    ...
    for _b in range(config.batch_size):
        _, _data, _label, _point_anno, _, _ = next(loader_iter)
        _data = _data.cuda()
        _label = _label.cuda()
...
def test(config, net, test_loader):
    with torch.no_grad():
        ...
        for i in range(len(test_loader.dataset)):
            _, _data, _label, _, vid_name, vid_num_seg = next(load_iter)
            _data = _data.cuda()
            _label = _label.cuda()
            ...
            for i in range(len(config.act_thresh_cas)):
                ...
                proposals = get_proposal_oic(seg_list, cas_temp, score_np, pred, config.scale,config.feature_fps, config.feat_stride, config.window_len, _lambda=config.lamb, gamma=config.gamma)     
...
def base_function(args):
    ...
    #############################################################################################################
    # Prepare the dataset
    train_dataset = ThumosFeature(data_path=config.data_path, feat_stride= config.feat_stride, feat_offset = config.feat_offset, feature_fps=config.feature_fps ,mode='train',seed=config.seed)
    ...
    if not train_flag:
        net.load_state_dict(torch.load( "./base_models/base_model.pkl"))

    test(config, net, eval_train_loader)
    print("Performance of Base Proposals on the train set:---------------------")
    base_proposals_path = os.path.join(config.output_path, 'base_proposals.json') #generated by test 
    check_performance(base_proposals_path, config.gt_path, mode="train")
    #############################################################################################################
    # Generate pseudo-labels   
    train_filter_proposals(base_proposals_path,config.point_dict_path, config.output_path)

    print("Performance of Pseudo-labels on the train set:---------------------")
    check_performance(config.PL_path, config.gt_path, mode="train")
